...
const App: React.FC = () => {
  ...
  return (
    <div className="min-h-screen font-sans selection:bg-[#9C7A3C]/20 selection:text-[#5A4628]">
      {/* Mandatory Mission Statement - Persistent Top Banner */}
      ...
      <header className="bg-white/90 border-b border-[#9C7A3C]/10 py-8 px-16 flex flex-col lg:flex-row justify-between items-center sticky top-[44px] md:top-[48px] z-50 backdrop-blur-3xl gap-8">
        ...
        <nav className="flex flex-wrap justify-center gap-2 bg-[#F4F3EE] p-1.5 rounded-full border border-[#9C7A3C]/10">
          <button 
            ...
            className={`px-6 py-3.5 rounded-full text-[9px] font-black uppercase tracking-[0.2em] transition-all ${activeTab === 'vacuum' ? 'bg-[#9C7A3C] text-white shadow-lg' : 'text-[#6A6A6A] hover:text-[#9C7A3C]'}`}
          >
            {language === 'ta' ? 'வெற்றிடங்கள்' : 'Vacuums'}
          </button>
          <button 
            onClick={() => setActiveTab('decay')}
            className={`px-6 py-3.5 rounded-full text-[9px] font-black uppercase tracking-[0.2em] transition-all ${activeTab === 'decay' ? 'bg-[#9C7A3C] text-white shadow-lg' : 'text-[#6A6A6A] hover:text-[#9C7A3C]'}`}
          >
            {language === 'ta' ? 'சிதைவு' : 'Decay'}
          </button>
          <button 
            onClick={() => setActiveTab('simulator')}
            className={`px-6 py-3.5 rounded-full text-[9px] font-black uppercase tracking-[0.2em] transition-all ${activeTab === 'simulator' ? 'bg-[#9C7A3C] text-white shadow-lg' : 'text-[#6A6A6A] hover:text-[#9C7A3C]'}`}
          >
            {language === 'ta' ? 'தூண்டுதல்' : 'Simulator'}
          </button>
          <button 
            onClick={() => setActiveTab('insights')}
            className={`px-6 py-3.5 rounded-full text-[9px] font-black uppercase tracking-[0.2em] transition-all ${activeTab === 'insights' ? 'bg-[#7B2D2D] text-white shadow-lg' : 'text-[#6A6A6A] hover:text-[#7B2D2D]'}`}
          >
        ...
        <div className="hidden lg:flex bg-[#1E1E1E] px-8 py-3.5 rounded-full items-center gap-5 text-white shadow-xl border border-[#9C7A3C]/20">
           ...
           <div className="flex flex-col">
              <span className="text-[8px] font-black text-[#6A6A6A] uppercase tracking-widest leading-none mb-1">Terminal Secure</span>
              <span className="text-[9px] font-black text-[#9C7A3C] uppercase tracking-widest">Level 07 Access</span>
           </div>
        </div>
      </header>
      ...
      <main className="pt-20 px-8 md:px-16 pb-32 max-w-[1400px] mx-auto">
        <section className="animate-in fade-in slide-in-from-bottom-8 duration-700">
           {activeTab === 'vacuum' && <MandateVacuumIdentifier language={language} />}
           {activeTab === 'decay' && <AccountabilityAuditTerminal language={language} />}
           {activeTab === 'simulator' && <CounterfactualSimulator language={language} />}
           {activeTab === 'insights' && <GovernanceInsights language={language} />}
           {activeTab === 'audit' && <ForensicJourneyTrace language={language} />}
        </section>
      </main>

      {/* Governance Analysis Footer */}
